...
interface FactoringOfertasTableProps {
  factoringId: string;
...
const getOfertaStatus = (factura: Factura) => {
  const estado =
    factura.factoringIsOfertme?.toLowerCase() ||
    factura.ofertaFactoring?.estado?.toLowerCase() ||
    "";
  ...
  switch (estado) {
    ...
    case "enviada":
      return {
        label: estado === "enviada" ? "enviada" : "Activa",
        color: "var(--color-fg-accent-primary)",
        bg: "var(--color-bg-accent-secondary)",
      };
    default:
      return {
        label: estado || "Con oferta",
        color: "var(--color-fg-accent-primary)",
        bg: "var(--color-bg-accent-secondary)",
      };
  }
...
const FactoringOfertasTable = ({
  factoringId,
  onMetaChange,
}: FactoringOfertasTableProps) => {
  const { getFacturasConOfertasByFactoringId, loading } = useFacturas();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setError(false);
      const response = await getFacturasConOfertasByFactoringId({
        page: meta.page,
        limit: meta.limit,
        factoringId,
        ...
        {loading ? (
          ...
          >
            ...
            <Typography
              ...
              sx={{ color: "var(--color-fg-default-secondary)", fontWeight: 500 }}
            >
              Aún no tienes facturas con ofertas.
            </Typography>
            <Typography
              variant="body2"
            ...
            {meta.lastPage > 1 && (
              ...
              >
                <FormControl size="small" sx={paginationSelectSx}>
                  <InputLabel id="factoring-ofertas-limit-label">
                    Filas por página
                  </InputLabel>
                  <Select
                    labelId="factoring-ofertas-limit-label"
...
export default FactoringOfertasTable;
