...
interface FactoringGruposTableProps {
  factoringId: string;
...
const FactoringGruposTable = ({
  factoringId,
  onMetaChange,
}: FactoringGruposTableProps) => {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const { getFacturaGruposFactoring, loading } = useFacturaGrupos();
  ...
  const fetchGrupos = useCallback(async () => {
    ...
    try {
      const porcentaje = filters.porcentajeFinanciamiento
        ...
        : undefined;
      const response = await getFacturaGruposFactoring({
        factoringId,
        page: meta.page,
        limit: meta.limit,
      ...
      console.error("Error fetching factura grupos for factoring:", err);
      setGrupos([]);
      setLoadError("No se pudieron cargar los grupos. Intente nuevamente.");
    }
    // getFacturaGruposFactoring is recreated every render
  ...
  const emptyMessage = loadError
    ...
    : hasActiveFilters
      ...
      : "Los grupos de cotización disponibles aparecerán aquí";
                        ...
                        <TableCell>
                          <Tooltip title="Ver grupo">
                            <IconButton
                              size="small"
                              onClick={() =>
                                navigate(getFacturaGrupoFactoringPath(grupo.id), {
            ...
            {meta.lastPage > 1 && (
              ...
              >
                <FormControl size="small" sx={paginationSelectSx}>
                  <InputLabel id="factoring-grupos-limit-label">
                    Filas por página
                  </InputLabel>
                  <Select
                    labelId="factoring-grupos-limit-label"
...
export default FactoringGruposTable;
